	...
	def process(self, annotations, relations):
		...
		for target_id, modifiers in sorted_modifiers.items():
			...
			for rule_id, modifiers_in_rule in match_checker[source_type].items():
				if modifiers_in_rule == modifiers:
					matched_conclusion_types.add(inference_map[source_type][rule_id])
...
def read_csv_rules(file, lower=True, header_lines=0, delimiter=','):
	rows = []
	pwd = os.getcwd()
	if pwd not in file:
